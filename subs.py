...
def find_media_files(directory, age_in_hours=None):
...
def select_directory_dialog():
    ...
    try:
        ...
        directory = filedialog.askdirectory(title="Selecteer de map met video bestanden")
        ...
        print("Tkinter is niet beschikbaar. Geef de map op via de command line.")
        return None
...
def main():
    ...
    parser.add_argument('directory', nargs='?', default=None, help='Directory to scan for media files (optional)')
    ...
    # If no directory is provided in the command line, open a directory selection dialog
    if args.directory is None:
        print("Geen map opgegeven, openen van mapkeuze dialoog...")
        directory = select_directory_dialog()
        if directory is None:
            print("Geen map geselecteerd. Beëindiging.")
            return 1
        args.directory = directory
    
    # Verify the input directory exists
    if not os.path.isdir(args.directory):
        print(f"Error: Map '{args.directory}' bestaat niet.")
        return 1
    ...
    print(f"Scanning for media files ({time_message})...")
    media_files = find_media_files(args.directory, args.hours)
    
    if not media_files:
        no_files_message = "No media files found"
        if args.hours is not None and args.hours > 0:
            no_files_message += f" modified in the last {args.hours} hours"
        no_files_message += f" in '{args.directory}'."
